...
const LogisticalFAQ = () => {
    // State to manage which FAQ is open for the Billing FAQ section
    const [openBillingFAQ, setOpenBillingFAQ] = useState(null);

    // Function to toggle FAQ for the Billing section
    const toggleBillingFAQ = (index) => {
        setOpenBillingFAQ(openBillingFAQ === index ? null : index);
    };

    return (
        <div className='flex flex-col lg:flex-row gap-6 lg:gap-10 mx-4 sm:mx-8 md:mx-[60px] lg:mx-[100px] mt-20'>
            {/* Travel FAQ Section */}
            <div className='text-left w-full lg:w-1/2 rounded-lg px-4 sm:px-8 py-6 border-2 border-solid border-[#f1f5f9] bg-gray-500'>
                {/* Placeholder for Travel FAQ content */}
            </div>
